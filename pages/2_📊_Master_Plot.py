...
with open('css/master_plot.css', 'r') as file:
        css = file.read()
        st.markdown(f'<style>{css}</style>', unsafe_allow_html=True)
...
with main_container:
    with left:
        ...
        if 'prev_point' not in st.session_state:
            ...
            st.session_state.prev_prev_point = []
            st.session_state.prev_prev_prev_point = []
        # selected_points = []
        # prev_point = selected_points
        # prev_prev_point = prev_point
        fig.update_layout({"uirevision": "foo"}, overwrite=True)
        selected_points = plotly_events(fig, hover_event=True, click_event=True)
        ...
        if selected_points:
            ...
            # print("prev:",selected_points,st.session_state.prev_point,st.session_state.prev_prev_point)
            # st.session_state.prev_prev_prev_point = st.session_state.prev_prev_point
            # st.session_state.prev_prev_point = st.session_state.prev_point
            # st.session_state.prev_point = selected_points
            # print(selected_points,st.session_state.prev_point,st.session_state.prev_prev_point,st.session_state.prev_prev_prev_point)
            # if st.session_state.prev_point == selected_points:
            #     if st.session_state.prev_prev_point == st.session_state.prev_point and st.session_state.prev_prev_prev_point==st.session_state.prev_prev_point:
            #         st.success("OKK")
            #         print("ok_______________________________")
            #         st.session_state.prev_point = []
            #         st.session_state.prev_prev_point = []
            #         st.session_state.prev_prev_prev_point = []
